...
def initiate_dialog():
    ...
    MyDialog.create_user_message(target=RobotSimulator.target, robot=RobotSimulator.robot)
...
def interact_with_gpt():
    ...
    while config.TURN < config.TURN_MAX:
        config.TURN += 1
    ...
    logger.info('机器人在规定探索次数内没有接近目标，探索终止')
